...
def main():
    ...
    #This for loop will loop 100 times to select a reward based on the randomly generated epsilon value
    for loop_num in range(100):
        val = -1
        reward = -1

        # print(f"\nEpsilon: {e:.3f}\n")
        # if the  randomly generated value is less than e, randomly choose treatment 
        if random.random() < e:
            val = random.choice([1, 2, 3])
            #if the choice is i randomly choose reward value from the given reward value
            # val 1 is green treatment, val 2 is blue treatment, val 3 is red treatment
            if val == 1:
                ...
                NRed += 1
        #if randomly chosen value is greater than e, take the greedy method to select the next reward
        else:
            #take the maximum value from the average reward of all treatment
            ...
            if highest == QGreen:
                ...
                reward = np.random.choice(red)
                NRed += 1
        #calculate the average reward including the new reward
        if val == 1:
        ...
        #store the average reward of all treatment
